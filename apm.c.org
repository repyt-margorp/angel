...
#define APM_BASE_ARRAY_INVERSE_CASING
 ...
 *	Casing is necessary for the convergence of star(*)
 *	algorithm, because the system do never stop if A is the
 *	min value. To exclude casing, convergence type sharp(#)
 *	can by applied, where the system converges in the case of
 *	the min value A. Also sharp(#) type algorithm converges
 *	into the required newton inverse denoted by at-mark(@)
 *	except for A is 2^n, but it accordingly requires to
 *	detect whether A is 2^n, which is more costly.
 ****************************************************************
 */
 ...
 */
#ifdef APM_BASE_ARRAY_INVERSE_CASING

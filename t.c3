	...
	int *res)
{
	...
	if(link(link_param, lst, &lst_link)) {
		return 1;
	}
...
int header_size(struct header *head)
{
	...
	size = (char *)USED(head)->link - (char *)&USED(head)[1];
...
int split(struct header *head, int size, int length)
{
	...
	link = (struct pointer *)((char *)USED(head)
		+ sizeof(struct used) + size);
	tail = (struct header *)((char *)link
...
int main()
{
	...
	split(pool, 1024, 8); proc = pool; pool = pool->next;
